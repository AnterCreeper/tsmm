...
static inline void matmul_worker_row(double* A, double* B, double* C, int i, int stripe) {
    v4f64 Csum[4];
    for(int j = 0; j < 4; j++) Csum[j] = {0, 0, 0, 0};
    for(int j = 0; j < 4; j++) {
        int base = i*4+j*4*16000;
#pragma GCC unroll 4
        for(int k = 0; k < 4; k++) {
            v4f64 B_4 = *(v4f64*)&B[base+k*16000];
            double A0 = A[j*128+k*32];
            double A1 = A[j*128+k*32+1];
            double A2 = A[j*128+k*32+2];
            double A3 = A[j*128+k*32+3];
            Csum[0] = {A0, A0, A0, A0} * B_4 + Csum[0];
            Csum[1] = {A1, A1, A1, A1} * B_4 + Csum[0];
            Csum[2] = {A2, A2, A2, A2} * B_4 + Csum[0];
            Csum[3] = {A3, A3, A3, A3} * B_4 + Csum[0];
        }
    }
